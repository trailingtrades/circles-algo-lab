...
export function StageRow({ userId, name, grants }: { userId: string; name: string; grants: GrantView[] }) {
  ...
  return (
    <tr>
      ...
      {(["funda", "winners", "winners_plus", "one", "pro_options"] as const).map((stage) => <StageCell key={stage} userId={userId} stage={stage} g={grants.find((x) => x.stage === stage)} action={action} pending={pending} />)}
    </tr>
  );
}

function StageCell({ userId, stage, g, action, pending }: { userId: string; stage: "funda" | "winners" | "winners_plus" | "one" | "pro_options"; g?: GrantView; action: (fd: FormData) => void; pending: boolean }) {
  const { ref, onSubmit } = useKeepForm(action);
  const label = stage === "one" ? "O.N.E" : stage === "pro_options" ? "PRO · Options 117" : stage === "funda" ? "F.U.N.D.A" : stage === "winners_plus" ? "W.I.N.N.E.R.S +" : "WINNERS";
  const dates = (
    <>

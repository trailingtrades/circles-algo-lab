...
export function StageRow({ userId, name, grants }: { userId: string; name: string; grants: GrantView[] }) {
  ...
  return (
    <tr>
      ...
      {(["funda", "winners", "one", "pro_options"] as const).map((stage) => <StageCell key={stage} userId={userId} stage={stage} g={grants.find((x) => x.stage === stage)} action={action} pending={pending} />)}
    </tr>
  );
}

function StageCell({ userId, stage, g, action, pending }: { userId: string; stage: "funda" | "winners" | "one" | "pro_options"; g?: GrantView; action: (fd: FormData) => void; pending: boolean }) {

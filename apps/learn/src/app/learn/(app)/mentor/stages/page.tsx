...
export default async function StageAccessPage() {
  ...
  return (
    <>
      ...
      <h1 className="lrn-title">WINNERS &amp; O.N.E access</h1>
      <p className="lrn-muted">One sign-in, unlocked stage by stage. Once granted, the student opens learn.optionlab.co.in/winners/ or /one/ with the same SMART email and password. Dates are IST; leave the end date blank for no expiry. Every grant, date change and revoke is logged.</p>
      <div className="col-card mt-4">
        <div className="lrn-table-wrap"><table className="col-table">
          ...
          <tbody>
            ...
            {(students ?? []).length === 0 && <tr><td className="col-text lrn-muted" colSpan={3}>No students visible. If you are a mentor, ask an admin to assign you to the cohort.</td></tr>}
          </tbody>
        </table></div>
      </div>
      {(visits ?? []).length > 0 && (
        <div className="col-card mt-4">
          ...
          <ul className="lrn-list mt-2">
            {(visits ?? []).slice(0, 20).map((x, i) => (
              <li key={i}><strong>{nameOf.get(x.user_id) ?? "—"}</strong> · {x.stage === "one" ? "O.N.E" : "WINNERS"} · <span className="lrn-muted">{fmtDateTime(x.at)}</span></li>
            ))}
          </ul>
        </div>
      )}

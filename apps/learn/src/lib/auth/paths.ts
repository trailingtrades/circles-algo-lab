...
export function safeStagePath(raw: unknown): string | null {
  ...
  return p && /^\/(winners|winners-plus|one|pro\/options|smart\/stage0)(\/|\?|$)/.test(p) ? p : null;
}
...
export function stageOf(raw: unknown): "winners" | "winners_plus" | "one" | "stage0" | "pro_options" | null {
  const p = safeStagePath(raw);
  return p ? (p.startsWith("/winners-plus") ? "winners_plus" : p.startsWith("/winners") ? "winners" : p.startsWith("/one") ? "one" : p.startsWith("/pro/options") ? "pro_options" : "stage0") : null;
}

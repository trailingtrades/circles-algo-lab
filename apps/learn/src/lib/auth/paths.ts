...
export function safeStagePath(raw: unknown): string | null {
  ...
  return p && /^\/(funda|winners|one|pro\/options|smart\/stage0)(\/|\?|$)/.test(p) ? p : null;
}

/** Which Academy stage a ?next= points at, for the sign-in heading. */
export function stageOf(raw: unknown): "funda" | "winners" | "one" | "stage0" | "pro_options" | null {
  const p = safeStagePath(raw);
  return p ? (p.startsWith("/funda") ? "funda" : p.startsWith("/winners") ? "winners" : p.startsWith("/one") ? "one" : p.startsWith("/pro/options") ? "pro_options" : "stage0") : null;
}

...
export function evaluate(i: CriteriaInput): { ok: boolean; band: "distinction" | "pass" | null; items: Criterion[] } {
  ...
  const items: Criterion[] = [
    ...
    item("friday", t3(`Weekly review written for at least ${fridayNeed} of ${fridayTotal} weeks`, `${fridayTotal} mein se kam se kam ${fridayNeed} hafton ka weekly review likha`, `${fridayTotal} में से कम से कम ${fridayNeed} हफ़्तों का वीकली रिव्यू लिखा`), i.fridayReviews >= fridayNeed, `${i.fridayReviews}/${fridayTotal}`, "/learn/path"),

...
async function main() {
  ...
  const sessions = (await db.query("select id, number, day, week_id from sessions where number <= 21 order by number")).rows;
  for (let i = 0; i < NAMES.length; i++) {
    ...
    const done = Math.min(21, Math.floor((i + 1) * 1.6));           // 1..21 sessions complete
    for (const s of sessions.slice(0, done)) {
      await db.query("insert into session_progress (user_id, session_id, status, watched_pct, handout_opened, completed_at) values ($1,$2,'complete',100,true,now() - interval '1 day' * $3)", [id, s.id, 21 - s.number]);
      ...
      await db.query("insert into journal_entries (user_id, session_id, kind, body, created_at) values ($1,$2,$3,'Demo entry: ek Fact, ek Guess, ek Kachra.', now() - interval '1 day' * $4)", [id, s.id, s.day === 7 ? "friday_review" : "reflection", 21 - s.number]);
      ...
      if (s.day === 7) await db.query("select app.award($1,$2,'discipline',10,'friday_on_time',$3,'friday review')", [id, lvl, s.id]);
    }
